import type { Stack } from "./types";

export const androidStack: Stack = {
  name: "Android",
  icon: "images/stack-android.webp",
};

export const typescriptStack: Stack = {
  name: "Typescript",
  icon: "images/stack-typescript.png",
};

export const javascriptStack: Stack = {
  name: "Javascript",
  icon: "images/stack-javascript.png",
};

export const linuxServerStack: Stack = {
  name: "Linux Server",
  icon: "images/stack-linux-server.png",
};

export const microsoftSqlStack: Stack = {
  name: "Microsoft SQL Server",
  icon: "images/stack-microsoft-sql-server.png",
};

export const playwrightStack: Stack = {
  name: "Playwright",
  icon: "images/stack-playwright.png",
};

export const nextJsStack: Stack = {
  name: "Next.js",
  icon: "images/stack-nextjs.png",
};

export const expressJsStack: Stack = {
  name: "Express.js",
  icon: "images/stack-expressjs.png",
};

export const pythonStack: Stack = {
  name: "Python",
  icon: "images/stack-python.png",
};

export const reactStack: Stack = {
  name: "React",
  icon: "images/stack-react.png",
};

export const vueStack: Stack = {
  name: "Vue",
  icon: "images/stack-vue.png",
};

export const rubyOnRailsStack: Stack = {
  name: "Ruby on Rails",
  icon: "images/stack-ruby-on-rails.png",
};

export const golangStack: Stack = {
  name: "Golang",
  icon: "images/stack-golang.png",
};

export const redisStack: Stack = {
  name: "Redis",
  icon: "images/stack-redis.png",
};

export const tailwindStack: Stack = {
  name: "Tailwind",
  icon: "images/stack-tailwind.png",
};

export const javaStack: Stack = {
  name: "Java",
  icon: "images/stack-java.png",
};

export const cStack: Stack = {
  name: "C Programming Language",
  icon: "images/stack-c.png",
};